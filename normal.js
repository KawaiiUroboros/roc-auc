(function() {
    ...
    function pnorm_lower(mean, stddev, x) {
        ...
        prob = 1 - sum * (Math.exp(-z_score * z_score * .5)
                          / Math.sqrt(2 * Math.PI));
    ...
    function normal_dist(mean, stddev) {
        var x = xlim[0], y, xe = xe = xlim[1], f, p, data = [];
        q = 2 * stddev * stddev;
        for (; x <= xe; x += 0.05) {
            y = Math.exp(-Math.pow(x - mean, 2) / q)
                / Math.sqrt(Math.PI * q);
            data.push({'x': x, 'y': y});
        }
    ...
    function axes(height) {
        ...
        svg.append('line')
            .attr('class', 'normal-threshold')
            .attr('y1', 0)
            .attr('y2', height);
        svg.append('text')
            .attr('class', 'normal-threshold-prob')
            .attr('y', -10);
        svg.append('text')
            .attr('class', 'normal-threshold-label')
            .text('threshold');
    }
    ...
    function render_controls() {
        var d = container.append('div'), s, h, num_steps = 100;
        s_mean = -1;
        s_stddev = 1;
    ...
    normal.init = function(id, xlimits, width, height, margin) {
        ...
        container.on("mousemove", function() {
            var coord = d3.mouse(this), cx = coord[0],
            lx, tpr, fpr;
            cx = cx - margin;
            if (cx < 0)
                cx = 0;
